...
public class Student {
    ...
    public static Optional<Student> create(final String firstName, final String lastName, final int indexNumber) {
        ...
        try {
            int newId = QueryExecutor.createAndObtainId(sql, args);
            return Student.findById(newId);
    ...
    public static Optional<Student> findByIndexNumber(final int indexNumber) {
        ...
        try {
            ResultSet results = QueryExecutor.read(sql, args);
            if (results.next()) {
                return Student.findById(results.getInt("id"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
